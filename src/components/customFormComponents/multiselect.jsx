...
const MultiSelect = ({ options, value, onChange }) => {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <div className="border rounded-md p-2 cursor-pointer" onClick={() => setIsOpen(!isOpen)}>
        ...
        {value.map((val) => (
          <span key={val} className="inline-block bg-blue-500 text-white rounded-full px-2 py-1 text-sm mr-2">
            ...
            <button type="button" className="ml-1 text-white" onClick={(e) => {
              e.stopPropagation();
              handleRemove(val);
            }}>
              &times;
      ...
      {isOpen && (
        <div className="absolute z-10 mt-2 w-full bg-muted/100 border rounded-md shadow-lg">
          {options.map((option) => (
            <div key={option.value} className="flex items-center p-2 cursor-pointer hover:bg-gray-500" onClick={() => handleSelect(option.value)}>
              <input type="checkbox" checked={value.includes(option.value)} readOnly className="mr-2" />
              {option.label}
            </div>
          ))}
        </div>
