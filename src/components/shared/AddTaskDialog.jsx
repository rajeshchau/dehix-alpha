          ...
          <form onSubmit={handleFormSubmit}>
            <div className="space-y-4 p-4">
              ...
              <div className="flex items-center gap-3">
                <label htmlFor="taskStatus" className="w-1/3 text-sm font-medium mb-1">
                  Task Status
                </label>
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button
                      variant="ghost"
                      className={`w-full border rounded-md text-left ${
                        errors.taskStatus ? 'border-red-500' : ''
                      }`}
                    >
                      {formData.taskStatus
                        ? {
                  ...
                  <DropdownMenuContent>
                    <DropdownMenuItem onSelect={() => handleInputChange('NOT_STARTED')}>
                      Not Started
                    </DropdownMenuItem>
                    <DropdownMenuItem onSelect={() => handleInputChange('ONGOING')}>
                      On Going
                    </DropdownMenuItem>
                    <DropdownMenuItem onSelect={() => handleInputChange('COMPLETED')}>
                      Completed
                    </DropdownMenuItem>
                  </DropdownMenuContent>
                </DropdownMenu>
              ...
              <div className="flex items-center gap-3">
                <label htmlFor="freelancer" className="w-1/3 text-sm font-medium mb-1">
                  Freelancer
                </label>
                <Popover>
                  <PopoverTrigger className="overscroll-y-none" asChild>
                    <Button
                      variant="ghost"
                      className={`w-full border rounded-md text-left ${
                        errors.freelancer ? 'border-red-500' : ''
                      }`}
                    >
                            ...
                            <div className="space-y-2 px-4 py-2">
                              {filteredFreelancers.map((freelancer) => (
                                <div
                                  ...
                                  onClick={() => {
                                    setSelectedFreelancer(freelancer.userName);
                                  ...
                                  <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                      ...
                                      <div className="text-xs text-gray-500">
                                        {truncateDescription(freelancer.email, 20)}
                                      </div>
                                    </div>
                                  </div>
