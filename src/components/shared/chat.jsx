...
const CardsChat = ({ conversation }) => {
  ...
  const [showFormattingOptions, setShowFormattingOptions] = useState(false); // Toggle formatting options
  ...
  async function sendMessage(conversation, message, setInput) {
    try {
      setIsSending(true);
  ...
  async function handleCreateMeet() {
    try {
      ...
      const meetLink = response.data.meet 
      const message = {
        senderId: user.uid,
