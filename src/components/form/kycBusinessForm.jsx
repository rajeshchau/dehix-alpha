...
const kycFormSchema = z.object({
  ...
  frontImageUrl: z
    .union([typeof window !== 'undefined' ? z.instanceof(File) : z.unknown(), z.string().url(), z.null()])
    .optional(),
  backImageUrl: z
    .union([typeof window !== 'undefined' ? z.instanceof(File) : z.unknown(), z.string().url(), z.null()])
    .optional(),
...
export function KYCForm({ user_id }) {
  ...
  async function onSubmit(data) {
    ...
    try {
      ...
      if (data.frontImageUrl instanceof File) {
        ...
        const response = await axiosInstance.post('/register/upload-image', frontFormData, {
      ...
      if (data.backImageUrl instanceof File) {
        ...
        const response = await axiosInstance.post('/register/upload-image', backFormData, {
          headers: { 'Content-Type': 'multipart/form-data' },
        });
      ...
      if (data.liveCaptureUrl instanceof File) {
        ...
        const response = await axiosInstance.post('/register/upload-image', liveFormData, {
          headers: { 'Content-Type': 'multipart/form-data' },
        });
    ...
    <Card className="p-10">
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          <div>
            KYC Status{' '}
            <Badge className={`text-xs py-0.5 ${kycBadgeColors[kycStatus] || ' '}`}>
